...
GV.Config.spatialstore = "http://localhost/spatialstore/";//EXTERNAL_SERVER_ADDRESS
...
GV.TRIPLESTORE_API = "http://localhost/openrdf-sesame/repositories/oberwesel";//EXTERNAL_SERVER_ADDRESS
GV.TRIPLESTORE_GUI = "http://localhost/openrdf-workbench/repositories/oberwesel";//EXTERNAL_SERVER_ADDRESS
...
IO.deleteFeatures = function() {
	...
	if (confirm("Sollen " + size(selfids) + " Geometrien wirklich entfernt werden?")) {
		...
		deleteGeoObject(geoURIs);//edit felix
	}
}
...
IO.saveScreenshot = function(sid,fid,edited) {
	...
	if (!edited) {
		geo2pundit(fid,img,"3D Geometrie No. "+selected.getName());
	}
