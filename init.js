...
GV.loadPundit = function() {
	loadPundit();
	GV.Spatialstore.punditLoaded = true;
}
...
GV.Info = function() {
	...
	this.clientError = function(xmlhttp) {
	...
	this.select = function(features) {
		...
		if (s == 0) {
			if (GV.info.allDisplayed)
				span.innerHTML = "Please select a feature.";
			else
		...
		if (s > 1) {
			span.innerHTML = "You have selected " + s + " features.";
