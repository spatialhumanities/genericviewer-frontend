...
THCTag = {
	...
	getXPointerFromSelection: function () {
		return THCTag.getXPointerFromSelectionWithAction("annotation_request");
	},

	getSelectedText: function (action) {
		var content = THCTagUtil.extractContentFromRange(THCTagCore.getSelectedRange());
		var contents = new Array();
		contents[0] = content;
		THCTagUtil.writeMessage(action, null, contents);
	},
	...
	addByXPointer: function (xPointer) {
		...
		this.addByXPath(startUrl, splittedXPointer[0][0], splittedXPointer[0][1],
	...
	// show annotation
	show: function (startUrl, startXPath, startOffset, endXPath, endOffset, goToAnchor) {
		...
		THCTagUtil.writeMessage("selection_request", xpointers, '');
		if (goToAnchor == undefined) goToAnchor = true;
		THCTagCore.Annotate.showNoteById(startXPath, startOffset, endXPath, endOffset, goToAnchor);
	...
	hide: function () {
		...
		THCTagUtil.writeMessage("selection_request", null, '');
	},
...
THCTagCore = {
	...
	getPrimaryTopicUri: function () {
		//alert("getting primary topic...");
		var matchingElements = THCTagUtil.getElementsByAttribute(document.documentElement, "link", "rel", "foaf:primarytopic");
		var url = '';
		if (matchingElements.length != 0) {
			url = matchingElements[0].getAttribute("href");
			//alert("primary topic found: " + url);
		} else {
			//alert("No primary topic found.");
	...
	calculateOffset: function (offset, node) {
		...
		if (THCTagCore.isTHCTag(node)) {
			...
			if (!sibling) {
				...
				return offset;
				//return this.getPreviousSiblingNodeWithSameName(parent);
...
THCTagCore.Annotate = {
	...
	deselectionLinkbutton: function (deselectId) {
		...
		icon.setAttribute("src", "http://metasound.dibet.univpm.it/svn/scripts/trunk/thctag/images/close.png");
		icon.setAttribute('onclick', "THCTagCore.Annotate.deselectFragment(); return false;");
		icon.setAttribute("style", "cursor:pointer; border:none;padding:2px;margin:0px;position:static;display:inline;");
	...
	selectionLinkButton: function (elementName, startUrl, startXPath, startOffset, endXPath, endOffset) {
		...
		imageIcon.setAttribute("src", "http://metasound.dibet.univpm.it/svn/scripts/trunk/thctag/images/baloon.png");
		...
		imageIcon.setAttribute("onClick", "THCTag.show(\"" + startUrl + "\", \"" + startXPath + "\", " + startOffset + ", \"" + endXPath + "\" , " + endOffset + ", false); return false;");
	...
	selectionAnchor: function (startUrl, startXPath, startOffset, endXPath, endOffset) {
		...
		var xpointer = THCTagCore.XPath.getXPointerString(startUrl, startXPath, startOffset, endXPath, endOffset);
		var anchorId = THCTagUtil.decodeUrl(xpointer + "-anchor");
	...
	showNoteById: function (startXPath, startOffset, endXPath, endOffset, goToAnchor) {
		...
		if (INLINE_HIGHLIGHTING) {
			 ...
			 */
			var selectionId = THCTagUtil.decodeUrl(xpointer + "-sel");
			...
			//var icon = THCTagUtil.getElementsByAttribute(document.body, "a", "id", xpointerId);
			...
			if (goToAnchor) window.location.hash = anchorId;
...
THCTagUtil = {
	...
	writeMessage: function (action, xpointers, contents) {
		...
		var xmlResult = '';
		xmlResult += '<annotator_message action=\'' + action + '\'>\n';
		...
		if (xpointers != null && xpointers.length > 0) {
			...
			for (var i = 0; i < contents.length; i++) {
				...
				xmlResult += this.getNotifyMessage('', content);
			}
	...
	getNotifyMessage: function (xpointer, content) {
		...
		xmlResult += '<fragment>\n';
		xmlResult += '<type>text</type>\n';
		if (xpointer != '') {
			xmlResult += '<xpointer>' + xpointer + '</xpointer>\n';
			xmlResult += '<container_uri>' + THCTagCore.getUrlFromXPointer(xpointer) + '</container_uri>';
		} else {
			//xmlResult += '<container_uri>' + THCTagCore.getPrimaryTopicUri() + '</container_uri>';
			var locationNoFregment = window.location.href.split("#")[0];
			if (locationNoFregment == undefined) {
				locationNoFregment = window.location.href;
			}
			xmlResult += '<container_uri>' + locationNoFregment + '</container_uri>';
		}
		xmlResult += '<content>' + Encoder.htmlEncode(content, false) + '</content>';
		xmlResult += '<context_url>' + THCTagUtil.removeFragmentFromUrl(window.location.href) + '</context_url>\n';
		xmlResult += '</fragment>\n';
...
window.status = "<annotator_message action='script_ready'/>";
