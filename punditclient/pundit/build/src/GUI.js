...
dojo.declare("pundit.GUI", pundit.BaseComponent, {
	...
	opts: {
		...
		minWindowHeight: 130,
		maxWindowHeight: 500 Felix*/
	},

	constructor: function (options) {
				//options.debug=true;//Felix
		/*todo: diesen Block nach möglichkeit löschen*/
		var self = this;
		...
		//Felix self._currentWindowHeight = 0;
		...
		//Felix self.resizeSemlibWindow(self.opts.initialWindowHeight);
...
//todo: Login von der Anzeige trennen. Rollout des Menüs in jedem Fall, Login geschieht woanders im GenericViewer.

		_PUNDIT.requester.onLogin(function (data) {
			self.show();
			dojo.query('#pundit-gui-topbar').addClass('pundit-logged-in').removeClass('pundit-logged-off');
		});
//Hier werden die Buttons für das Ausfahren des Annotation Writers gesetzt.
...
//todo Platzierung der Pundit-GUI eingebettet in  DIV der Seite
	initWindow: function () {
		...
		sw += '<div id="pundit-gui" class="pundit-base tundra startup pundit-disable-annotation">';//Felix: pundit-stop-wheel-propagation weg
		/*Container für SuggestionPanel*/
		sw += '<div id="felix-pundit-gui-suggestion-container"></div>';
		sw += '<div id="pundit-gui-resize-handler"></div>';

		sw += '<ul id="pundit-gui-panel-container" class="pundit-horizontal-list">';
		sw += '<li id="pundit-gui-left" class=""><div id="pundit-gui-preview" class="pundit-moreinfo-panel"></div></li>';//Felix: pundit-stop-wheel-propagation weg
...
//edit IBR: Einsetzen der Pundit-GUI innerhalb des Seiten-Content-Bereiches, in dediziertem DIV. War 'body' s.a. andere unten
		...
		dojo.query('#genv_pundit_container').append(aw);

		//Felix self.resizeAnnotationWindow();
	...
	show: function () {
		var self = this;
			//todo: Pundit initial nicht ausgefahren.
		//dojo.addClass('pundit-gui', 'pundit-hidden');
		dojo.removeClass('pundit-gui', 'startup');
		//dojo.query('#genv_pundit_container').addClass('pundit-gui-hidden'); Felix

		//dojo.addClass('pundit-aw', 'pundit-hidden');
		dojo.removeClass('pundit-aw', 'startup');
		//dojo.query('#genv_pundit_container').addClass('pundit-aw-hidden');Felix
...
*/
	...
	// Hide both top and annotation windows //todo: Setzen von pundit-hidden herausnehmen.
	hide: function () {
		dojo.removeClass('pundit-gui', 'pundit-hidden pundit-shown');
		dojo.addClass('pundit-gui', 'startup');
		//dojo.query('#genv_pundit_container').removeClass('pundit-gui-hidden pundit-gui-shown');

		//dojo.removeClass('pundit-aw', 'pundit-hidden pundit-shown');
		dojo.addClass('pundit-aw', 'startup');
		//dojo.query('#genv_pundit_container').removeClass('pundit-aw-hidden pundit-aw-shown');
	},
...
//todo: Mittlere und Linke Bedienoberfläche herausnehmen.=> Evtl. vorerst nur über das CSS.
		// Center panel tabs
		var centerTabs = {
			//'#tabItems': {
			'#pundit-tab-page-items': {
				...
				filter: function () {
					semlibItems.showOnlyItems(dojo.query('#pundit-items li'));
				}
			},
			//'#tabMyItems': {
			'#pundit-tab-my-items': {
...
*/
		...
		dojo.query('#genv_pundit_container').toggleClass('pundit-gui-hidden');
		dojo.query('#genv_pundit_container').toggleClass('pundit-gui-shown');
		self.resizeAnnotationWindow();
	...
	toggleAnnotationWindow: function () {
		var a = dojo.query('#pundit-aw'),
				b = dojo.query('#genv_pundit_container'),
				self = this;
...
//todo: Stylevorgaben rausnehmen, alles in der CSS beschreiben?
		/*dojo.style('pundit-gui', 'height', h + 'px'); Felix
...
//todo: alles rausnehmen, was die Platzierung der GUI beeinträchtigt.
...
//                dojo.style('pundit-aw', 'display', 'none');
	...
	initNotebookShareDialog: function (id) {
		...
		var dialog = new dijit.Dialog({//todo: Checken: wozu ist das da? Außerdem muss der Code innerhalb der Pundit-DIV abgelegt werden.
			style: "width: 600px"
		});
