...
/*edit Felix				dojo.style(dojo.query('#' + self._id + ' ul.pundit-fp-content-list')[0], {
...
/*edit Felix				dojo.style(dojo.query('#' + self._id + ' ul.pundit-fp-content-list')[0], {
	...
	initBaseHTML: function () {
		...
		/*edit Felix: Suggestion Panel wird in pundit-tc-dnd-container positioniert*/
		var panel_container=(self.name=='subj-suggestions' | self.name=='pred-suggestions' |self.name=='obj-suggestions')?'pundit-tc-container':'genv_pundit_container';
		...
		var c = '<div id="' + self._id + '" class="pundit-base pundit-fp pundit-hidden pundit-disable-annotation">';//Felix: pundit-stop-wheel-propagation weg
		c += '  <div class="pundit-fp-header"><span class="pundit-fp-title">' + self.title + '</span>';
		c += '     <span class="pundit-fp-close pundit-icon-close">CLOSE</span>';
		...
		c += '          </div>';
		c += '      </li>';
		c += '      <li class="pundit-fp-preview-list fillheight">';//Felix: pundit-stop-wheel-propagation weg
		c += '          <div class="pundit-fp-container fillheight">';//Felix: pundit-stop-wheel-propagation weg
		...
		dojo.query('#'+panel_container).append(c);//dojo.query('#'+panel_container).append(c);
	},
			...
			dojo.connect(window, 'onmousemove', function (e) {
				if (self.moving === true) {
					...
					if (e.clientY > 0 && e.clientY < window.innerHeight - 10) {
						dojo.stopEvent(e);/*todo Felix: Prüfen, ob diese inline styles gelöscht werden können.*/
...
/*Felix: Close Button für Dropdown-Menüs => Soll durch Click auf umgebende Leisten ersetzt werden.*/
