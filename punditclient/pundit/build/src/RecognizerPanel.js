...
dojo.declare("pundit.RecognizerPanel", pundit.ResourcesPanel, {
	...
	initHTML: function () {
		...
		c += '          <div class="pundit-pane"><ul class="pundit-rp-tags pundit-items pundit-view-list showsRemoveButton"></ul></div>';//Felix: pundit-stop-wheel-propagation weg
		c += '      </div>';
		c += '   </div>';
		...
		var rec=dojo.query('#' + self._id);
		dojo.query('#' + self._id + ' .pundit-rp-container-search').prepend(c);
		var recnu=dojo.query('#' + self._id);
		if (rec!=recnu)console.log(rec);
	},
	initDnD: function () {
		this.inherited(arguments);
		var self = this;
