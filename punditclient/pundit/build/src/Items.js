...
dojo.declare("pundit.Items", pundit.BaseComponent, {
	...
	init: function (name) {
		...
		si += '<li id="pundit-tab-filter-' + this.name + '-entities">Terms</li>';//Felix: Ist für mittleres Panel. Hier fehlen ansonsten noch die Geometrien
		...
		si += '<div id="pundit-tab-content' + this.name + '" class="pundit-tab-content"><ul id="pundit-' + this.name + '" class="pundit-items pundit-view-list"></ul></div>';//Felix: pundit-stop-wheel-propagation weg
		si += '</div>';
...
//var myitems = semlibMyItems.getItemsFromTerm('', acceptedTypes, [], ns.geometric_object_class); bei Aufruf durch Objekt
	...
	getItemsfromUris: function (uris, rdftypes) { //edit Felix: Methode zur Wiedergabe der gerade angezeigten Geometrien. Filterung nach URIs und RDF-Typ
		...
		return self.filterItemsByRdftype(items, rdftypes);
		...
		for (var i = items.length; i--;) {
			...
			for (var j = items[i].rdftype.length; j--;) {
				for (var k = rdftypes.length; k--;) {
					...
					if (items[i].rdftype[j] === rdftypes[k]) {//Felix. BUG: Wenn nur ein rdftype vorgegeben ist, wird hier statt dieses Typs der letzte Buchstabe des Typnamen referenziert
