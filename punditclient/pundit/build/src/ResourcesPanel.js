/*Felix: Angepasste Version, die für das Subj-Obj-Pred- SuggestionPanel im TripleComposer spezialisiert ist*/
/*todo: Wird für alle Panels (auch für die mittlere GUI) aufgerufen. Konstruktur für andere Panels also deaktivieren*/
/*
...
dojo.declare("pundit.ResourcesPanel", pundit.BasePanel, {
	...
	constructor: function (options) {
		...
		self.maxShownItems = options.maxShownItems || 5;
		self.suggestionPanels = options.suggestionPanels || {
			...
			properties: {
				label: 'Properties',//Felix: Braucht man wohl nicht. Properties werden im Pred-Panel sowieso geladen (und zwar ohne Label)
				expanded: true
			}
		};
		...
		self.panels = {};
		for (var attrname in self.suggestionPanels) {
			self.panels[attrname] = self.suggestionPanels[attrname];
		}
		for (var attrname in self.namedEntitiesSources) {
			self.panels[attrname] = self.namedEntitiesSources[attrname];
		}
		...
		if (_PUNDIT.config.vocabularies.length !== 0) {
			_PUNDIT['vocab'].onVocabsLoaded(function () {
				...
				self.log('Vocab loaded. Init HTML Resource panel')//Felix: Hier werden die Einträge für (statische) Vokabulare gesetzt.
				for (var i in _PUNDIT.vocab.vocabs) {
					self.suggestionPanels[i] = {label: _PUNDIT.vocab.vocabs[i].label};//Name des Vokabulare. Wo sind die Inahlteß
					self.panels[i] = i;
				}
	...
	initHTML: function () {
		...
		self.log('Resource Panel Constructor');
		self.expandedClass;
		var c = '              <div class="pundit-rp-literal-panel pundit-hidden">';
		c += '                  <div class="pundit-rp-container-literal">';
		c += '                      <span class="pundit-pane-title pundit-rp-subtitle">Insert text:</span>';
		c += '                      <section contenteditable="true" class="pundit-rp-literal-text" ></section>';//Felix: pundit-stop-wheel-propagation weg
		...
		c += '                  <div class="pundit-rp-container-date">';
		c += '                      <span class="pundit-pane-title pundit-rp-subtitle">Date in "YYYY-MM-DD" format: </span><br/>';
		...
		c += '                      <span class="pundit-pane-title pundit-rp-subtitle"></span>';
		c += '                      <div>';
		c += '                          <input class="pundit-rp-search-input pundit-dialog-input" row="1" placeholder="Search" value="" type="text"/>';
		...
		for (var i in self.suggestionPanels) {
			...
			/*Edit Felix: Integration in Genv-Container durch BasePanel*/
			var xitem = dojo.query('#' + self._id);
		}
	...
	initBehaviors: function () {
		...
		for (var i in self.panels) {
			(function (_sp) {
				...
				/*edit Felix: Click-Handler für Drag&Drop vom SuggestionPanel zu den Triple-Slots. Auf das Vorschaubild erweitert nicht nur span.pundit-item-label*/
				beh['#' + self._id + '-suggestions-list-' + _sp + ' li.dojoDndItem'] = {
					'onclick': function (e) {
    ...
    *        })
	...
	load: function (itemsObject) {
		...
		for (var i in itemsObject) {//Felix: hier wird evtl. die Reihenfolge der items festgelegt.
	...
	/*Felix: Steuert Anzeige u.a. für Preview*/
...
//                }
	...
	createItemsObject: function (term) {
		...
		itemsObject = {//todo: Prüfen, ob hier nicht auch textrepository und geometries stehen müssen.
			myitems: {
				label: 'My Items',
				items: myitems
			}/*, Felix
	...
	rankItems: function (container, word) { //Felix: Rankitems: Hier werden myItems sortiert.
		var rankedItems = [];
		for (var i in container.map) {
			rankedItems.push([i, container.map[i].data.label.score(word)]);
		}
		rankedItems.sort((function (a, b) {
			return b[1] - a[1]
		}));
