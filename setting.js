GV.Setting = function() {
	...
	this.changeFloorplanViewpoint = function() { floorplan.changeViewpoint(GV.setting.spatialstore.getSpatialcontext().getViewpoint()); }
	this.changeFloorplan = function() { floorplan.changeFloorplan(GV.setting.spatialstore.getSpatialcontext().getFloorplan()); floorplan.changeViewpoint(GV.setting.spatialstore.getSpatialcontext().getViewpoint()); }
	this.changeViewerPanorama = function() { viewer.changePanorama(GV.setting.spatialstore.getSpatialcontext().getViewpoint().getPanorama()); }
	this.changeViewerFeatures = function() { viewer.changeFeatures(); floorplan.changeFeatures(); }
	this.changeViewerMeasurement = function() { viewer.updateMeasurement(); floorplan.changeFeatures(); }
	...
	var azim = 0;
	var elev = Math.PI/2;
	this.getAzim = function() { return azim; }
	this.getElev = function() { return elev; }
	this.rotate = function(dAzim,dElev) {
		azim += zoom*dAzim;
		elev -= zoom*dElev;
		if (elev > Math.PI) elev =  Math.PI;
		if (elev < 0) elev = 0;
		floorplan.update();
		viewer.update();
	}
	...
	this.getMeasurement = function() { return measurement; }
	...
	this.resize = function() {
		...
		document.getElementById("tabs-overview").style.width = tabs2width + "px";
		document.getElementById("tabs-sparql").style.width = tabs2width + "px";
		viewer.resize();
	}
}
